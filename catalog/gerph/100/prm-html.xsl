...
<localdb:definition-names type="swi" singular="SWI" />
<localdb:definition-names type="vector" singular="Vector" />
...
<xsl:comment>
  ...
  This document was created with the HTML stylesheet updated (11 Apr 2002).
</xsl:comment>
<xsl:apply-templates />
...
<!-- definition sections -->
<xsl:template match="swi-definition|service-definition|upcall-definition|command-definition|entry-definition|sysvar-definition|error-definition|message-definition|vector-definition" mode="contents">
...
<xsl:choose>
 <xsl:when test="@internal = 'yes'">
 <dd><p>This Wimp Message is for internal use only. You must not use it in
        your own code.</p></dd></xsl:when>
 <xsl:otherwise>
  <dd><xsl:value-of select="@description"/></dd>
...
<xsl:variable name="hrefto">
 ...
 <xsl:if test="(substring(@href,1,1) != '?') and
               (@href != '') and
               (not(contains(@href,':')))">
...
<xsl:variable name="refname"><xsl:value-of select="@name"/></xsl:variable>
<xsl:variable name="refreason"><xsl:value-of select="@reason"/></xsl:variable>
